...
export default function Home({ books, cart, setCart, wishlist, setWishlist }) {
      ...
      <div className="content-wrapper">
        ...
        <section className="books-section">
          <h2 className="section-title">
            {filtered.length === 0 ? 'No books found' : `Found ${filtered.length} book${filtered.length !== 1 ? 's' : ''}`}
          </h2>
                  ...
                  <div className="book-card-body">
                    ...
                    <div className="book-card-rating">
                      <div className="stars-display-small">
                        {[1, 2, 3, 4, 5].map(star => (
                          <span key={star} className={`star-small ${star <= Math.round(avgRating) ? 'filled' : ''}`}>
                            ★
                          </span>
                        ))}
                      </div>
                      {avgRating > 0 && <span className="rating-number">({avgRating.toFixed(1)})</span>}
                    </div>
                    
                    <p className="book-card-price">${book.price?.toFixed(2) || 'N/A'}</p>
                    <p className="book-card-category">{book.category}</p>
                    
                    <div className="book-card-actions">
                      <button
                        className={`btn btn-small ${isInCart ? 'btn-disabled' : 'btn-primary'}`}
                        onClick={() => !isInCart && setCart([...cart, book])}
                        disabled={isInCart}
                      >
                        {isInCart ? '✓ In Cart' : 'Add to Cart'}
                      </button>
                      <button
                        className={`btn btn-small btn-icon ${isInWishlist ? 'btn-active' : ''}`}
